...
The repository has completed **Phase 1 — repository initialization** and is implementing
**Phase 2 — fundamental data model**. It contains a minimal FastAPI application, typed SQLAlchemy
models, Alembic migrations, append-only history protection, a PostgreSQL Docker Compose service,
and real-PostgreSQL integration tests.

Phase 3 and later phases are not implemented. In particular, there are no task-transition rules,
LLM integrations, autonomous runtime agents, executable tools, skills, MCP integrations, or
